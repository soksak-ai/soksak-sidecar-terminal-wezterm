...
//
// 예산 게이트는 계약 코퍼스를 반복해 안정적인 처리량을 재므로 한 번에 수십 초가 든다. feed
// 경로를 고치는 동안에는 그 반복이 너무 길다. 이 하니스는 같은 코퍼스를 같은 미러에 먹이되
// feed 만 잰다 — 고친 것이 벌었는지를 몇 초 안에 답한다.
//
...
// 실전은 한 덩어리가 아니다 — 입력 스트림은 조각으로 배달된다. 조각마다 무는 비용(엔진 진입·
