...
### Fixture history — width-two wrapping

The qualification baseline was **6 of 7**. Fixture ② (`cjk_width`) was RED because the
...
80-column grid and a scrollback one row short. The declared reference state requires wrap-before-placement.

The engine revision declared in `Cargo.toml` enforces the required wrap-before-placement invariant.
With the fixture unchanged, the suite
is 7 of 7, the engine's own test suite is unregressed, and the fixtures repeat 20/20
without flaking. Release input is the immutable dependency URL and revision declared in `Cargo.toml`;
